...
TestDate::testcheckLeapYear(void)
{
   //todo add test code using ASSERT macro(s)
     CPPUNIT_ASSERT_EQUAL(2001,getYear());
}
...
{
   //todo add test code using ASSERT macro(s)
     CPPUNIT_ASSERT_EQUAL("JANUARY,getMonthName());
}
...
int main(int argc, char* argv[])
{
    ...
    testresult.addListener (argc, argv);
    ...
    testrunner.addTest(argv[1],CPPUNIT_NS::TestFactoryRegistry::getRegistry().TestDate());
    testrunner.run(argc, argv);

    // todo: output results in xml format
      CPPUNIT_NS::CompilerOutputter outputter(argc, argv);
      outputter.write();
